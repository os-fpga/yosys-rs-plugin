...
#include "../pmgen/rs-dsp-pm.h"

void create_rs_dsp(rs_dsp_pm &pm)
{
    auto &st = pm.st_rs_dsp;

    log("Checking %s.%s for RS DSP inference.\n", log_id(pm.module), log_id(st.mul));
    ...
    Cell *cell = st.mul;
    if (cell->type == ID($mul)) {
        log("  replacing %s with RS_DSP cell.\n", log_id(st.mul->type));

        cell = pm.module->addCell(NEW_ID, ID(RS_DSP));
        pm.module->swap_names(cell, st.mul);
    } else
        log_assert(cell->type == ID(RS_DSP));

    // RS_DSP Input Interface
    SigSpec A = st.sigA;
...
struct RSDspPass : public Pass {
    RSDspPass() : Pass("rs_dsp", "rs: map multipliers") {}
    void help() override
    {
        //   |---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|---v---|
        log("\n");
        log("    rs_dsp [options] [selection]\n");
        log("\n");
        log("Map multipliers ($mul/RS_DSP) and multiply-accumulate ($mul/RS_DSP + $add)\n");
        log("cells into rs DSP resources.\n");
        log("Pack input registers (A, B, {C,D}), pipeline registers\n");
        log("({F,J,K,G}, H), output registers (O -- full 32-bits or lower 16-bits only); \n");
        log("and post-adder into into the RS_DSP resource.\n");
        log("\n");
    ...
    {
        log_header(design, "Executing rs_DSP pass (map multipliers).\n");
        ...
        for (auto module : design->selected_modules())
            rs_dsp_pm(module, module->selected_cells()).run_rs_dsp(create_rs_dsp);
    }
} RSDspPass;
