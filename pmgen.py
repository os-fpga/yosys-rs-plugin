...
with open(outfile, "w") as f:
    ...
    for index in range(len(blocks)):
        ...
        if block["type"] == "match":
            ...
            if(prefix == "rs_dsp_macc" and block["cell"]=="mux"): # Awais: emulate "out = subtr?acc-mul:acc+mul" for RS DSP MACC against EDA-348
                print("  dict<index_{}_key_type, vector<index_{}_value_type>> index_{}_mux;".format(index, index, index), file=f)
            elif ((prefix == "rs_dsp_macc" and block["cell"]=="add")): # Awais: emulate "out = subtr?acc-mul:acc+mul" for RS DSP MACC against EDA-348
    ...
    for index in range(len(blocks)):
        ...
        if block["type"] == "code":
            ...
            if len(block["if"]):
                for expr in block["if"]: 
                    if ((expr.find("neg")!=-1) and (expr.find("nullptr")!=-1) and prefix == "rs_dsp_macc"): # Awais: emulate "out = subtr?acc-mul:acc+mul" for RS DSP MACC against EDA-348
